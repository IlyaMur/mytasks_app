...
**Содержание**
  ...
  - [Установка](#установка)
    - [Сборка в Docke-контейнере](#сборка-в-docker-контейнере)
...
Изменить (опционально, будет работать и с настройками по умолчанию) параметры подключения в файле `.env` и скрипте миграции `database/import.php`
